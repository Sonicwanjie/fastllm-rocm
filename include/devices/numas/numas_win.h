// Windows-compatible numas.h replacement
// Same interface, no Linux numa.h/numaif.h dependency

#ifndef FASTLLM_NUMAS_H
#define FASTLLM_NUMAS_H

#include "fastllm.h"

namespace fastllm {
    struct MachineNumaInfo {
        int numaCnt = 1;
        int threads = -1;
        std::vector<std::vector<int>> cpuIds;
        MachineNumaInfo();
    };

    struct NumaConfig {
        int numaCnt = 1;
        int threads = -1;
        std::vector<std::vector<std::pair<int, int>>> numaToCpuDict;
        std::vector<int> threadIdToNumaDict;

        NumaConfig();
        NumaConfig(int threads, AliveThreadPool *pool, MachineNumaInfo *info);
    };

    void bind_to_cpu(int cpu_id);
    void bind_to_numa_node(int node_id);
    void set_numa_mempolicy(int node_id);
    void* allocate_aligned_numa(size_t size, int node);
    void free_aligned_numa(void* aligned_ptr, size_t size);
    void* allocate_aligned(size_t size);
    void free_aligned(void* aligned_ptr, size_t size);
    void* allocate_pinned_numa(size_t size, int node);
    void free_pinned_numa(void* ptr, size_t size);
}
#endif
