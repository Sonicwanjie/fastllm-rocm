...
namespace fastllm {
    Gemma4Model::Gemma4Model() {
        ...
        weight.embeddingNames.insert("model.language_model.embed_tokens_per_layer.weight");
        ...
        weight.linearNames = {
            ...
            "model.language_model.layers.*.per_layer_input_gate.weight",
            "model.language_model.layers.*.per_layer_projection.weight"
        };
    }
    ...
    void Gemma4Model::InitParams() {
        ...
        global_num_key_value_heads = (val.empty() || val == "None" || val == "null" || val == "0") ? num_key_value_heads : atoi(val.c_str());
        ...
        val = GetDictValue(d, "text_config.hidden_size_per_layer_input", "hidden_size_per_layer_input", "0");
        hidden_size_per_layer_input = atoi(val.c_str());

        val = GetDictValue(d, "text_config.num_kv_shared_layers", "num_kv_shared_layers", "0");
        num_kv_shared_layers = atoi(val.c_str());
    ...
    int Gemma4Model::ForwardTextFromHiddenStates(const Data &inputIds,
        ...
        int sharedKVFullSourceLayer = -1;
        if (num_kv_shared_layers > 0) {
            int firstShared = block_cnt - num_kv_shared_layers;
            for (int li = firstShared - 1; li >= 0; li--) {
                if (layer_types[li] == 1 && sharedKVFullSourceLayer < 0) {
                    sharedKVFullSourceLayer = li;
                }
                if (layer_types[li] == 0 && sharedKVSlidingSourceLayer < 0) {
                    sharedKVSlidingSourceLayer = li;
                }
            }
        }
        for (int i = 0; i < block_cnt; i++) {
            ...
            // KV shared layers: determine source layer
            int kvSourceLayer = i;
            bool isKVShared = (num_kv_shared_layers > 0 && i >= block_cnt - num_kv_shared_layers);
            if (isKVShared) {
                kvSourceLayer = isFullAttn ? sharedKVFullSourceLayer : sharedKVSlidingSourceLayer;
            }
            ...
            Data &pastKey = pastKeyValues[kvSourceLayer].first, &pastValue = pastKeyValues[kvSourceLayer].second;
            if (GetKVCacheInCPU()) {
                pastKey.lockInCPU = true;
                pastValue.lockInCPU = true;
            } else {
                pastKey.ToDevice(q.dataDevice);
                pastValue.ToDevice(q.dataDevice);
            }
            ...
            if (pleEnabled) {
                ...
                pleSlice.Reshape({pleSlice.dims[0], pleSlice.dims[1], hidden_size_per_layer_input});
                
                Data pleGate, pleOut, pleProj;
    ...
    std::vector <int> Gemma4Model::ForwardBatch(int batch,
        ...
        for (int i = 0; i < block_cnt; i++) {
            ...
            if (!isKVShared) {
                ...
                int ktotal = 0;
                for (int b = 0; b < batch; b++) {
                    Split(k, 1, ktotal, ktotal + seqLens[b], curKs[b]);
                    Split(v, 1, ktotal, ktotal + seqLens[b], curVs[b]);
                    ktotal += seqLens[b];
                }
            ...
            for (int b = 0; b < batch; b++) {
                auto &curQ = curQs[b];
                Data &pastKey = *pastKeyValues[b * block_cnt + kvSourceLayer].first;
                Data &pastValue = *pastKeyValues[b * block_cnt + kvSourceLayer].second;
